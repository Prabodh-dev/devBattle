...
const api: AxiosInstance = axios.create({
  baseURL: `${NORMALIZED_API_URL}/api`,
  headers: {
...
export const userAPI = {
  ...
  respondToFriendRequest: async (
    ...
    const response = await api.put(`/friends/request/${requestId}/respond`, {
      action,
    });
    return response.data;
  },
...
export const leaderboardAPI = {
  ...
  getBattleLeaderboard: async (page = 1, limit = 50) => {
    const response = await api.get("/leaderboard/battles", {
      params: { page, limit },
    });
    return response.data;
  },
  ...
  getContestLeaderboard: async (page = 1, limit = 50) => {
    const response = await api.get("/leaderboard/contests", {
      params: { page, limit },
    });
    return response.data;
  },
...
export const judgeAPI = {
  submitCode: async (data: {
    ...
    const JUDGE_URL =
      process.env.NEXT_PUBLIC_JUDGE_URL || "http://localhost:8000";
